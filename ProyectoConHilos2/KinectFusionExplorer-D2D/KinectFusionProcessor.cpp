        ...
        else
        {
            if (m_bKinectFusionInitialized && m_bHaveValidCameraParameters)
            {
                ...
                else if (bResetReconstruction)
                {
                    ...
                    // Despertar el hilo
                    bAutomaticSave = true;
                    SavePose("filePoses2.txt");
                    //INuiFusionColorMesh* mesh = nullptr;
                    //HRESULT hr = CalculateMesh(&mesh);
                    //m_Saver lo puedo declarar como un puntero local aquí, reservar con new un espacio en memoria
                    //para la clase KinectFusionSaver, creo el hilo y llamo a saveMesh. Cuando se guarde el mesh
                    //hago delete al puntero para no ocupar tanta memoria.
                    //m_Saver = new CKinectFusionSaver;
                    //m_Saver->StartProcessing();
                    //m_Saver->SaveMesh(mesh);
                    HRESULT hr = InternalResetReconstruction();
                    if (hr != S_OK) {
                        SetStatusMessage(L"Failed to reset reconstruction.");
                    }
...
bool KinectFusionProcessor::CheckForReset()
{
    float roll, pitch, yaw, trans;

    yaw = atan2(m_worldToCameraTransform.M12, m_worldToCameraTransform.M11);
    pitch = atan2(-m_worldToCameraTransform.M13, sqrt(pow(m_worldToCameraTransform.M23, 2) + pow(m_worldToCameraTransform.M33, 2)));
    roll = atan2(m_worldToCameraTransform.M23, m_worldToCameraTransform.M33);
    trans = sqrt(pow(m_worldToCameraTransform.M41, 2) + pow(m_worldToCameraTransform.M42, 2) + pow(m_worldToCameraTransform.M43, 2));
    if (abs(yaw) >= 0.3 | abs(pitch) >= 0.3 | abs(roll) >= 0.3 | abs(trans) >= 0.1)
    {   
        multiplica(&mthAnt.M11, &m_worldToCameraTransform.M11, &mthGlobal.M11);
        mthAnt = mthGlobal;
        return true;
    }
    return false;
}
void KinectFusionProcessor::multiplica(float* mthAnt, float* mth, float* mthRes) {
    int row, col, i;
...
void KinectFusionProcessor::SavePose(const char* filename) {
