    ...
    case WM_INITDIALOG:
        {
            ...
            if (FAILED(m_processor.SetWindow(m_hWnd, WM_FRAMEREADY, WM_UPDATESENSORSTATUS)) ||
                ...
                FAILED(m_processor.StartProcessing())) // Hilo de procesado
            {
                m_bInitializeError = true;
            }

            m_Saver.SetWindow(m_hWnd, WM_SAVED);
            m_Saver.StartProcessing(); // Hilo de guardado
...
void CKinectFusionExplorer::HandleCompletedFrame()
{
    ...
    if (m_processor.bAutomaticSave) {
        ...
        m_processor.SetParams(m_params);
        //Save Mesh en un hilo. Creo el objeto de la clase CKinectFusionSaver y llamo al metodo saveMeshFile 
        m_Saver.mesh = mesh;
        m_processor.bAutomaticSave = false;
...
/*HRESULT CKinectFusionExplorer::SaveMeshFile2(INuiFusionColorMesh* pMesh)
{
    HRESULT hr = S_OK;
